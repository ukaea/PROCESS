        ...
        for var in self.tracking_variables:
            if "." in var:
                ...
                try:
                    ...
                    logger.warning(
                        "%s is a dotted variable and must be in the form "
                        "OVERRIDINGNAME.VARIABLE",
                        var,
                    )
            ...
            if mfile_var_value.get_number_of_scans() > 1:
                logger.info(
                    "Only scan 1 will be tracked,"
                    f" but {var} has {mfile_var_value.get_number_of_scans()} scans."
                )
...
def get_line_colour(title: str):
    """Given a run title, return the cached colour or pick a colour and then cache it."""
...
class TrackedVariable:
    """
    Holds the variable history
    over all runs that produced the variable in their output.
    """
    ...
    def add_datapoint(self, title, data, message, commit_hash, timestamp):
        ...
        message:
            latest commit message when `title` was run at time `date` (possibly unique)
        commit_hash:
            latest commit hash when `title` was run at time `date` (possibly unique)
        timestamp:
            date of the specific run this tuple refers to (unique)
        """
        self._data.append((title, timestamp, data, message, commit_hash))

    def as_dataframe(self):
        """
        Convert our internal data representation of a variable's history as a dataframe.
...
class TrackedData:
    ...
    def _add_variables(self, json_file_data):
        ...
        # as all datapoints of this file will require them
...
def plot_tracking_data(database, tracked_variables):
    """
    Drives the processing of existing .json tracking files
    and then plotting of this processed data.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The current commit message. "
        "If not provided, the code attempts to query to Git repository.",
    )
    parser.add_argument(
        "--hash",
        type=str,
        default=None,
        help="The current commit hash. "
        "If not provided, the code attempts to query to Git repository.",
    )
    parser.add_argument(
        ...
        "See the description of DEFAULT_TRACKING_VARIABLES for details "
        " on formatting the strings in the list.",
    )
