...
$$
\texttt{fc} = \frac{\frac{1}{2\pi}eB_{\text{T}}}{m_{\text{e}}}
$$

$$
\texttt{fp} = \frac{1}{2\pi}\sqrt{\frac{n_{\text{e,19}}e^2}{m_{\text{e}}\epsilon_0}}
$$

Apply effective charge correction from GRAY study

$$
\texttt{xi_{CD}} = 0.18\left(\frac{4.8}{2+Z_{\text{eff}}}\right)
$$

$$
\texttt{effrfss} = \frac{\texttt{xi_{CD}}T_{\text{e}}}{3.27R_0n_{\text{e,19}}}
$$
                
For the O-mode case:

$$
\texttt{f_{cutoff}} = \texttt{fp}
$$

For the X-mode case:

$$
\texttt{f_{cutoff}} = 0.5\left(\texttt{fc}+\sqrt{\texttt{n_ecrh_harmonic}\times\texttt{fc}^2+4\texttt{fp}^2}\right)
$$
...
$$
\texttt{cutoff_{factor}} = 0.5\left(1+\tanh\left({\left(\frac{2}{a}\right)((\texttt{n_ecrh_harmonic}\times \texttt{fc} -\texttt{f_cutoff})/\texttt{fp -a })}\right)\right)
$$

$$
\text{Current drive efficiency [A/W]} = \texttt{effrfss} \times \texttt{cutoff_{factor}}
$$
