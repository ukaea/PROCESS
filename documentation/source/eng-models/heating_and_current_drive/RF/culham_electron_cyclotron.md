...
1. Local electron temperature $(\texttt{tlocal})$ is calculated using the `teprofile` method  
2. Local electron density $(\texttt{dlocal})$ is calculated using the `neprofile` method

3. Calculate the inverse aspect ratio `epsloc`.

$$
\texttt{epsloc} = \frac{1}{3A}
$$

4. Calculate the Coulomb logarithm for ion-electron collisions `coulog`.[^2]

$$
\texttt{coulog} = 15.2 - 0.5\log({\texttt{dlocal}}) + \log({\texttt{tlocal}})
$$
...
$$
\texttt{ecgam} = 0.25(\texttt{ecgam1} + \texttt{ecgam2} +\texttt{ecgam3} + \texttt{ecgam4})
 $$

7. Calculate the current drive efficiency by dividing `ecgam` by `(dlocal * rmajor)`.

$$
\text{Current drive efficiency [A/W]} = \frac{\texttt{ecgam}}{\texttt{dlocal} \times R_0}
$$
