...
for i in range(temp_grid.shape[0]):
    for j in range(temp_grid.shape[1]):
        j_scaling[i, j], _, _ = superconductors.itersc(
...
ax.set_title("EUTF4 Nb\u2083Sn Critical Current Density Surface at Zero Strain")
ax.view_init(elev=30, azim=45)  # Rotate the plot around the z-axis by 45 degrees
fig.colorbar(surf, shrink=0.5, aspect=10)

# Save the plot as a PNG file
plt.savefig("eutf4_Nb3Sn_zero_strain.png", dpi=1000)
...
fig_plotly.update_layout(
    title="EUTF4 Nb\u2083Sn Critical Current Density Surface at Zero Strain",
    scene={
...
fig_plotly.write_html("eutf4_Nb3Sn_zero_strain.html")
