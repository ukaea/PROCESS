...
temp_conductor = np.linspace(5.0, 40, 50)  # Temperature range (K)
b_conductor = np.linspace(4, 15, 50)  # Magnetic field range (T)
...
ax.set_title("2nd Generation REBCO Critical Current Density Surface at Zero Strain")
ax.view_init(elev=30, azim=45)  # Rotate the plot around the z-axis by 45 degrees
fig.colorbar(surf, shrink=0.5, aspect=10)

# Save the plot as a PNG file
plt.savefig("2nd_gen_rebco_zero_strain.png", dpi=1000)
...
fig_plotly.update_layout(
    title="2nd Generation REBCO Critical Current Density Surface at Zero Strain",
    scene={
...
fig_plotly.write_html("2nd_gen_rebco_zero_strain.html")
