...
contains
   ...
   subroutine constraint_eqn_079(tmp_cc, tmp_con, tmp_err, tmp_symbol, tmp_units)
      ...
      !! Equation for maximum CS field
      !! #=# pfcoil
      !! #=#=# fbmaxcs, bmaxoh, b_cs_peak_pulse_start, b_cs_limit_max
      !! and hence also optional here.
      !! Logic change during pre-factoring: err, symbol, units will be assigned only if present.
      ...
      !! (Note: original code has "bmaxoh/b_cs_peak_pulse_start |  peak CS field [T]".)
      use pfcoil_variables, only: fbmaxcs, b_cs_limit_max, b_cs_peak_pulse_start, bmaxoh
      implicit none
            real(dp), intent(out) :: tmp_cc
      real(dp), intent(out) :: tmp_con
      real(dp), intent(out) :: tmp_err
      ...
      tmp_cc     = 1.0D0 - fbmaxcs * b_cs_limit_max/max(bmaxoh, b_cs_peak_pulse_start)
      tmp_con    = b_cs_limit_max
      tmp_err    = max(bmaxoh, b_cs_peak_pulse_start) * tmp_cc
      tmp_symbol = '<'
      tmp_units  = 'A/turn'
