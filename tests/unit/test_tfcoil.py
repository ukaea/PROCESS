...
class CntrpstTestAsset(NamedTuple):
    ...
    :expected_p_cp_pump_cool: expected value of tfcoil_variables.p_cp_pump_cool after tfcoil.cntrpst routine has run
    :type expected_p_cp_pump_cool: float
    """
...
def test_cntrpst(cntrpst_asset, monkeypatch, reinitialise_error_module, tfcoil):
    ...
    assert pytest.approx(tfv.p_cp_pump_cool) == cntrpst_asset.expected_p_cp_pump_cool
