...
def fraction_of_speed_of_light(ratio_of_ke_to_re):
    ...
    """
    return (
        relativistic_energy_to_speed(ratio_of_ke_to_re * NEUTRON_MASS_AS_ENERGY)
        / SPEED_OF_LIGHT
    )
...
def test_scattering_matrix(group_structure: npt.NDArray[np.float64], atomic_mass: float):
    ...
    group_energy, _ = calculate_mean_energy_and_incident_bin(
        group_structure, incident_energy
    )
    scattering_matrix = scattering_weight_matrix(
        group_structure, group_energy, atomic_mass
    )
