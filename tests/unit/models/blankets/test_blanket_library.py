...
def test_liquid_breeder_properties_part_1(monkeypatch, blanket_library_fixture):
    ...
    monkeypatch.setattr(blanket_library_fixture.data.fwbs, "i_blkt_liquid_breeder_type", 0)

    blanket_library_fixture.liquid_breeder_properties()

    assert pytest.approx(blanket_library_fixture.data.fwbs.den_liq, rel=1e-3) == 1.052e4
    assert pytest.approx(blanket_library_fixture.data.fwbs.specific_heat_liq, rel=1e-3) == 195.0
    assert pytest.approx(blanket_library_fixture.data.fwbs.thermal_conductivity_liq, rel=1e-3) == -3.384
    assert pytest.approx(blanket_library_fixture.data.fwbs.dynamic_viscosity_liq, rel=1e-3) == 0.0155
    assert pytest.approx(blanket_library_fixture.data.fwbs.electrical_conductivity_liq, rel=1e-3) == 9.71e5

    assert pytest.approx(blanket_library_fixture.data.fwbs.b_mag_blkt, rel=1e-3) == (9.085, 4.458)
    assert pytest.approx(blanket_library_fixture.data.fwbs.hartmann_liq, rel=1e-3) == (7.189e3, 3.528e3)

    # Li - see [Lyublinski et al., 2009] for relavent equations
    monkeypatch.setattr(blanket_library_fixture.data.fwbs, "i_blkt_liquid_breeder_type", 1)

    blanket_library_fixture.liquid_breeder_properties()

    assert pytest.approx(blanket_library_fixture.data.fwbs.den_liq, rel=1e-3) == 504.0
    assert pytest.approx(blanket_library_fixture.data.fwbs.specific_heat_liq, rel=1e-3) == 2.833e6
    assert pytest.approx(blanket_library_fixture.data.fwbs.dynamic_viscosity_liq, rel=1e-3) == 1.051e112
    assert pytest.approx(blanket_library_fixture.data.fwbs.electrical_conductivity_liq, rel=1e-3) == 9.27e8
    assert pytest.approx(blanket_library_fixture.data.fwbs.b_mag_blkt, rel=1e-3) == (9.085, 4.458)
    assert pytest.approx(blanket_library_fixture.data.fwbs.hartmann_liq, rel=1e-3) == (2.7e-53, 1.3e-53)
...
def test_liquid_breeder_properties_part_2(monkeypatch, blanket_library_fixture):
    ...
    assert pytest.approx(blanket_library_fixture.data.fwbs.b_mag_blkt, rel=1e-3) == (8.999, 4.458)
