...
def test_hylbld(hylbldparam, monkeypatch, ife):
    ...
    ife.hylbld()
    with open("/home/ubuntu/PROCESS/hylbld.pickle", "wb") as f:
        import pickle

        pickle.dump(
            {
                "fwmatv": ife_variables.fwmatv.tolist(),
                "v1matv": ife_variables.v1matv.tolist(),
                "blmatv": ife_variables.blmatv.tolist(),
                "v2matv": ife_variables.v2matv.tolist(),
                "shmatv": ife_variables.shmatv.tolist(),
                "v3matv": ife_variables.v3matv.tolist(),
                "chmatv": ife_variables.chmatv.tolist(),
            },
            f,
        )
...
@pytest.mark.parametrize(
    ...
    (
        Bld2019Param(
            ...
            expected_chmatv=np.array([261.69466804402975, 0, 0, 0, 0, 0, 0, 0, 0]),
        ),
    ),
)
