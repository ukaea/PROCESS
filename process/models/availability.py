...
class Availability(Model):
    ...
    def avail(self, output: bool):
        ...
        if (
            ...
            if self.data.divertor.n_divertors == 0:
                uplanned = self.data.costs.t_blkt_replace_yrs / (
                    self.data.fwbs.life_blkt_fpy + self.data.costs.t_blkt_replace_yrs
                )
            else:
    ...
    def avail_output(self, uplanned, uutot):
        ...
        po.ovarre(
            ...
            if (
                ...
                elif self.data.costs.life_div_fpy < self.data.fwbs.life_blkt_fpy:
                    po.ovarre(
                        self.outfile,
    ...
    def calc_u_planned(self, output: bool) -> float:
        ...
        if self.data.divertor.n_divertors == 0:
            u_planned = mttr_blanket / (self.data.fwbs.life_blkt_fpy + mttr_blanket)
        else:
            #  Which component has the shorter life?
            if self.data.costs.life_div_fpy < self.data.fwbs.life_blkt_fpy:
    ...
    def divertor_lifetime(self):
        ...
        """
        if self.data.divertor.n_divertors == 0:
            return self.data.costs.life_plant
