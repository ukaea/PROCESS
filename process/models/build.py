...
class Build(Model):
    ...
    def _vertical_build_out(self, i_single_null: DivertorNumberModels):
        ...
        has_divertor = self.data.divertor.n_divertors > 0

        configuration = "Single null" if sn else "Double null"
        if not has_divertor:
            configuration = "Limiter"
        po.ocmmnt(self.outfile, f"{configuration} case")
    ...
    def calculate_vertical_build(self, output: bool):
        ...
        if self.data.divertor.n_divertors == 0:
            self.data.divertor.dz_divertor = 0.0
        ...
        # Output the divertor geometry
        divht = 0.0 if self.data.divertor.n_divertors == 0 else self.divgeom(output)
