...
def get_solution_from_mfile(neqns, nvars, wdir="."):
    ...
    table_sol = [m_file.data[f"itvar{var_no + 1:03}"].get_scan(-1) for var_no in range(nvars)]
    table_res = [m_file.data[f"normres{con_no + 1:03}"].get_scan(-1) for con_no in range(neqns)]
