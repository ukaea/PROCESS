    ...
    if cond_type == 1:
        ...
        if wp_shape == 0:
            ...
            for i in range(1, short_turns):
                axis.plot(
                    [
                        (wp_inner + dx_tf_wp_insulation),
                        (wp_inner - dx_tf_wp_insulation + dr_tf_wp_with_insulation),
                    ],
                    [
        ...
        if wp_shape == 1:
            ...
            axis.add_patch(
                Rectangle(
                    (
                        wp_inner + (0.5 * dr_tf_wp_with_insulation),
                        -(0.5 * wp_toridal_dxbig),
                    ),
                    (dr_tf_wp_with_insulation / 2) - (2 * dx_tf_wp_insulation),
                    wp_toridal_dxbig - (2 * dx_tf_wp_insulation),
                    color="blue",
                    label=(
                        f"Winding pack: \n{turns} turns \n{j_tf_wp:.4f} MA/m$^2$ \n$"
        ...
        if wp_shape == 2:
            ...
            x = [
                wp_inner,
                wp_inner,
                (wp_inner + dr_tf_wp_with_insulation),
                (wp_inner + dr_tf_wp_with_insulation),
            ]
            y = [
            ...
            x = [
                wp_inner + dx_tf_wp_insulation,
                wp_inner + dx_tf_wp_insulation,
                (wp_inner + dr_tf_wp_with_insulation - dx_tf_wp_insulation),
                (wp_inner + dr_tf_wp_with_insulation - dx_tf_wp_insulation),
            ]
            y = [
                (-0.5 * wp_toridal_dxsmall + dx_tf_wp_insulation),
...
def plot_iteration_variables(axis, m_file_data, scan):
    ...
    for n in range(1, n_itvars+1):
        ...
        axis.plot(0, n_plot, "s", color="lightgray", markersize=8, label="Lower Bound" if n_plot == 0 else "")

        # Plot the upper bound at x=1
        axis.plot(1, n_plot, "s", color="lightgray", markersize=8, label="Upper Bound" if n_plot == 0 else "")

        # Plot the final value as a vertical marker, normalised to upper bound
        norm_final = itvar_final / itvar_upper if itvar_upper != 0 else 0
        axis.plot(norm_final, n_plot, "o", color="red")
        ...
        axis.set_xlim(-0.5, 1.5)  # Normalised bounds
        axis.set_title("Iteration Variables and Bounds")
        
        

    legend_handles = [
        mpatches.Patch(color="lightgray", label="Bounds"),
        mpatches.Patch(color="red", label="Final Value", alpha=0.7),
    ]
...
def main_plot(
    ...
    plot_18 = fig7.add_subplot(121)
    plot_vertical_build(plot_18, m_file_data, colour_scheme)
    ...
    if m_file_data.data["i_tf_sup"].get_scan(scan) == 1:
        ...
        plot_19.set_position([0.05, 0.5, 0.8, 0.4])
        plot_tf_wp(plot_19, m_file_data, scan)

        # TF coil turn structure
        plot_20 = fig8.add_subplot(325, aspect="equal")
        plot_20.set_position([0.1, 0.1, 0.3, 0.3])
