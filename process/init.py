...
def check_process():
    ...
    if (
        ...
        )  # No bore,gapoh, ohcth iteration
        and (
            abs(
                fortran.build_variables.bore
        ...
        )  # TF stress model can't handle no bore
    ):
        raise ProcessValidationError(
            "Invalid stress model if bore + gapoh + ohcth = 0. Don't use constraint 31"
        )
    ...
    if fortran.tfcoil_variables.temp_tf_coil_cryo > fortran.tfcoil_variables.tftmp:
        raise ProcessValidationError("temp_tf_coil_cryo should be lower than tftmp")
