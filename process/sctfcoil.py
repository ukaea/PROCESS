...
class Sctfcoil:
    ...
    def stresscl(
        ...
        s_shear_max_tf = np.zeros((n_tf_layer,))
        ...
        for ii in range(n_tf_layer):
            ...
            if i_tf_tresca == 1 and i_tf_sup == 1 and ii >= i_tf_bucking + 1:
                s_shear_max_tf[ii] = s_shear_cea_tf_cond[ii_max]
            else:
                s_shear_max_tf[ii] = s_shear_tf[ii_max]

        # Constraint equation for the Tresca yield criterion

        sig_tf_wp = s_shear_max_tf[n_tf_bucking]
        # Maximum assumed in the first graded layer

        if i_tf_bucking >= 1:
            sig_tf_case = s_shear_max_tf[n_tf_bucking - 1]
        if i_tf_bucking >= 2:
            sig_tf_cs_bucked = s_shear_max_tf[0]
        # ----------------
    ...
    def out_stress(
        ...
        if tfcoil_variables.i_tf_tresca == 1 and tfcoil_variables.i_tf_sup == 1:
            po.write(
                self.outfile,
                f"  Shear (CEA Tresca) \t\t\t (MPa) \t\t {table_format_arrays(s_shear_max_tf, 1e-6)}",
            )
        else:
            po.write(
                self.outfile,
                f"  Shear (Tresca) \t\t\t (MPa) \t\t {table_format_arrays(s_shear_max_tf, 1e-6)}",
            )
