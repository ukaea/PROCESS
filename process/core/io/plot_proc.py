...
def poloidal_cross_section(
    ...
    plot_lower_divertor(axis, mfile, scan, colour_scheme)
...
def color_key(axis: plt.Axes, mfile: mf.MFile, scan: int, colour_scheme: Literal[1, 2]):
    ...
    labels = [
        ...
        ("PF coils", "none"),
        ("Divertor", DIVERTOR_COLOUR[colour_scheme - 1]),
    ]
...
def plot_detailed_lower_divertor(
    ...
    ) / 2 - 0.75  # Offset for curvature (doubled)
    ...
    ) / 2 - 2.25  # Offset for curvature (increased from 2.0)
    ...
    # Plot vertical line at plasma inner boundary
    axis.axvline(
        cumulative_radial_build("dr_fw_plasma_gap_inboard", mfile_data, scan),
        color="red",
        linestyle="--",
        linewidth=0.5,
        label="Plasma Inner Boundary (with plasma to wall gap)",
    )
    ...
    axis.axhline(
        z_plasma_x_point_lower - dz_xpoint_divertor - dz_divertor,
        color="green",
        linestyle="--",
        linewidth=1.5,
        label="Divertor bottom",
    )
    ...
    axis.grid(True, linestyle="--", alpha=0.3)
    axis.set_ylim(
        (z_plasma_x_point_lower - dz_xpoint_divertor - dz_divertor) * 1.1,
...
def plot_lower_divertor(axis: plt.Axes, mfile_data: mf.MFile, scan: int, colour_scheme: int):
    ...
    r_div_inner_strike = rmajor - rminor * 0.7
